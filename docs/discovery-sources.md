...
## 12. AI-слой обработки сигналов (free-LLM, multi-provider)

Цель: бесплатно прогонять сырые сигналы через LLM для классификации и
извлечения структуры оффера, с автоматическим fallback между бесплатными
провайдерами, чтобы суммарной free-квоты хватало на весь поток.

### Что уже есть (база, не переписывать)
- **`HeuristicClassifier`** (`pipeline/classify.py`) — правила/ключевики, inline
  в пайплайне, нулевая стоимость, гоняется на каждый сигнал.
- **`LLMClassifier`** — OpenAI-совместимый, **батчевый** (`cli reclassify`),
  strict-JSON (`response_format=json_object`), уже умеет ротацию **моделей** при
  429/quota (`settings.llm_model_chain`).
- **`get_classifier()`** включает LLM при заданном `AIRADAR_LLM_API_KEY`, иначе
  heuristic. Pipeline инлайн всегда heuristic (бесплатно), LLM — отдельным
  батч-проходом по сохранённым офферам (бережёт дневные квоты).
- **Ограничение:** один провайдер = один `base_url` + один ключ; ротация только
  между МОДЕЛЯМИ внутри него.

### Чего не хватает (цель этого слоя)
1. **Multi-provider fallback** — цепочка независимых провайдеров (свой base_url +
   ключ у каждого), а не модели внутри одного эндпоинта. Умножает суммарную
   бесплатную квоту и даёт отказоустойчивость. Это прямое обобщение текущей
   model-ротации на уровень провайдеров — `_complete()` уже крутит цикл, в него
   встаёт перебор провайдеров.
2. **Дешёвый LLM fast-filter** для пограничных случаев (heuristic-score 0.1–0.8):
   не жечь токены на очевидном, но не терять тонкие офферы, которые правила
   пропускают.
3. **Deep-extract на странице** — при пробе нового домена (`enrich.probe`)
   скармливать LLM текст `/pricing`|`/help` и тащить структурированный оффер
   (лимиты, срок, промокод, условия: карта/смс/регион/только-новые).

### Провайдеры (free tier, ориентир на июнь 2026 — проверять при регистрации)
Все — бесплатная регистрация без карты. Все, кроме Workers AI, **OpenAI-совместимы**
→ втыкаются в существующий `OpenAI(base_url=...)` клиент без нового кода.

| Провайдер | base_url | Роль в стеке | Лимит (ориентир) |
|-----------|----------|--------------|------------------|
| **Google AI Studio (Gemini)** | `…/v1beta/openai/` | deep-extract длинных страниц (1M контекст) | высокий TPM, ~100–1000 RPD по модели |
| **Groq** | `api.groq.com/openai/v1` | fast-filter (LPU, очень быстрый) | ~30 RPM, RPD урезали в 2026 (~1K на 70B) |
| **Cerebras** | `api.cerebras.ai/v1` | fast-filter fallback (быстрый батч) | ~30 RPM / ~14.4K RPD |
| **Mistral** | `api.mistral.ai/v1` | deep-extract fallback | 1 req/s, ~1B токенов/мес |
| **OpenRouter** | `openrouter.ai/api/v1` | универсальный fallback (`:free`/`openrouter/free`) | ~200 RPD на модель |
| **Cloudflare Workers AI** | binding `AI` в воркере | ОТДЕЛЬНО (edge), не пайплайн | ~10K neurons/day |

Корректировки к ресёрчу (важно):
- **Grok (xAI) — платный**, постоянного free-tier нет. В стек не берём.
- **Workers AI ≠ дроп-ин для пайплайн-классификатора.** Python-пайплайн крутится
  в **GitHub Actions**, а не в Worker'е; Worker только читает D1. Поэтому Workers
  AI — это опциональный **edge-эндпоинт** в `worker/src/index.ts` (классификация
  по запросу/на лету), отдельная фича (см. #35). Для батч-пайплайна берём
  OpenAI-совместимые провайдеры.
- **Gemini free-tier** недоступен в EU/UK/CH и может использовать промпты для
  обучения. Для публичных доменных сигналов приемлемо, но знать.

### Архитектура (3 тира, fallback по провайдерам)
```
сигнал
  ▼ Tier 0  prefilter (ключевики)          — 0 стоимость, отсев явного мусора
  ▼ Tier 1  HeuristicClassifier            — 0 стоимость, даёт score
     ├ score > 0.8 → принять (LLM не зовём)
     ├ score < 0.1 → отбросить
     └ 0.1‥0.8     → Tier 2
  ▼ Tier 2  LLM fast-filter   (Groq → Cerebras → OpenRouter)   — короткий промпт, JSON
  ▼ Tier 3  LLM deep-extract  (Gemini → Mistral)               — только на странице при пробе
  ▼ store (D1 / SQLite)
```
Провайдер-цепочка: при 429/quota/сетевой ошибке → следующий провайдер; если все
выдохлись → heuristic. Strict-JSON у всех (`response_format=json_object`).

### Где ключи (вписывается в централизацию токенов §13)
Все новые ключи опциональны — без них слой no-op (откат на heuristic):
- `.env` (VDS) и GitHub Secrets (CF-пайплайн):
  `AIRADAR_GEMINI_KEY`, `AIRADAR_GROQ_KEY`, `AIRADAR_MISTRAL_KEY`,
  `AIRADAR_CEREBRAS_KEY`, `AIRADAR_OPENROUTER_KEY`.
- Порядок цепочки — через `AIRADAR_LLM_PROVIDERS` (csv по приоритету) или дефолт
  в коде.
- Workers AI — не env-ключ, а binding `AI` в `wrangler.toml` (только если делаем #35).
- Порядок регистрации по отдаче: **Gemini → Groq → Mistral → Cerebras → OpenRouter.**

### Бэклог — AI-слой

| # | Пункт | Зачем | Блокер |
|---|-------|-------|--------|
| 30 | **Multi-provider LLM-цепочка** — обобщить `LLMClassifier`: перебор провайдеров (base_url+ключ+модель) вместо моделей; конфиг `AIRADAR_LLM_PROVIDERS` | x-кратная free-квота + отказоустойчивость | — (чистый инжиниринг) |
| 31 | **Tier-2 fast-filter** для пограничных prefilter-случаев (0.1‥0.8) | точность без жжёных токенов | 🔑 Groq/Cerebras (free) |
| 32 | **Tier-3 deep-extract** на `/pricing`/`/help` в `enrich.probe` (JSON-схема оффера) | оффер с пустой главной / SPA | 🔑 Gemini/Mistral (free) |
| 33 | **Структурированный оффер-объект** (JSON Schema: `amount/unit/trial_days/requires_card/requires_sms/region/new_users_only/steps[]/risk_flags[]`) — закрывает §11 #25 | авто-карточки как в TG, но раньше | — (инжиниринг) |
| 34 | **LLM-dedup/merge** одинаковых офферов из разных источников | чистый фид без дублей | — (инжиниринг) |
| 35 | (опц.) **Workers-AI edge-эндпоинт** `/internal/classify` в `worker/src/index.ts` | on-demand классификация на крае, без внешних API | — (инжиниринг + CF binding `AI`) |

Рекомендованный порядок внедрения: **#30 → #33 → #31 → #32** (#30 и #33 без
ключей, дают фундамент; #31/#32 включаются по мере появления free-ключей). #34 и
#35 — по желанию после замера отдачи (§11 #28).

---

## 13. Реестр токенов (все секреты в одном месте)

Единая точка правды по секретам. **Все ключи опциональны.** Без своего ключа
каждый коллектор/LLM-провайдер деградирует в no-op (коллектор молча пропускается)
или в эвристику (`reclassify` без LLM-ключей откатывается на `HeuristicClassifier`).
Поэтому проект запускается «из коробки» с пустыми секретами, а оператор добавляет
ключи **в последнюю очередь**, по мере регистрации на бесплатных тарифах.

> **Важно для CF / GitHub Actions:** имена repo-секретов должны **точно** совпадать
> с тем, как они указаны в `secrets.*` в `collectors.yml`. То есть секрет в GitHub
> должен называться буквально `AIRADAR_GROQ_API_KEY`, `AIRADAR_OPENROUTER_API_KEY`
> и т.д. Исключения — Cloudflare и Telegram-нотификации, которые в `collectors.yml`
> читаются из укороченных имён (`CF_API_TOKEN`, `CF_ACCOUNT_ID`, `TG_BOT_TOKEN`,
> `TG_CHAT_ID`). `AIRADAR_GITHUB_TOKEN` особый: если секрет пуст, воркфлоу
> автоматически подставляет встроенный `github.token` (`${{ secrets.AIRADAR_GITHUB_TOKEN || github.token }}`),
> так что GitHub-коллектор работает даже без ручной настройки.

Колонки: **VDS** — нужен ли ключ в `.env` на VDS/Docker · **CF (Actions)** — нужен
ли как GitHub repo-secret для часового пайплайна.

| Переменная | Что включает | Где взять (free) | VDS | CF (Actions) |
|------------|--------------|------------------|-----|--------------|
| `AIRADAR_GITHUB_TOKEN` | GitHub code search (§4.1), повышенные лимиты `github`/`github_lists` | github.com → Settings → Developer settings → PAT (classic, `public_repo`) | ✅ | ✅ (или авто `github.token`) |
| `AIRADAR_SEARCH_API_KEY` | `searchdorks` (Google CSE) | console.cloud.google.com → Custom Search API | ✅ | ✅ |
| `AIRADAR_SEARCH_CX` | ID поисковой системы для `searchdorks` | programmablesearchengine.google.com | ✅ | ✅ |
| `AIRADAR_YOUTUBE_API_KEY` | `youtube` (мультиязычный поиск, харвест ссылок из описаний) | console.cloud.google.com → YouTube Data API v3 | ✅ | ✅ |
| `AIRADAR_PH_TOKEN` | `ph_upcoming` (ProductHunt Upcoming GraphQL) | api.producthunt.com → OAuth app | ✅ | ✅ |
| `AIRADAR_TW_BEARER_TOKEN` | `twitter` keyword-search (иначе только Nitter fallback) | developer.twitter.com → Free tier | ✅ | ✅ |
| `AIRADAR_FOFA_KEY` | `fofa` favicon-hash поиск relay-панелей (§8.1) | fofa.info → личный кабинет (free квота крошечная) | ✅ | ✅ |
| `AIRADAR_FOFA_EMAIL` | email-аккаунт FOFA (идёт в паре с ключом) | fofa.info | ✅ | ✅ |
| `AIRADAR_TG_API_ID` | `telegram` ingest через Telethon (int, дефолт 0) | my.telegram.org → API development tools | 🖥 ✅ | ❌ (stream-only, см. §14) |
| `AIRADAR_TG_API_HASH` | `telegram` ingest через Telethon (пара к API_ID) | my.telegram.org | 🖥 ✅ | ❌ (stream-only) |
| `AIRADAR_LLM_API_KEY` | основной LLM-провайдер для `reclassify` (OpenAI-совместимый) | провайдер по выбору (см. §12) | ✅ | ✅ |
| `AIRADAR_GEMINI_API_KEY` | LLM fallback — Google AI Studio (Gemini, deep-extract) | aistudio.google.com → Get API key | ✅ | ✅ |
| `AIRADAR_GROQ_API_KEY` | LLM fallback — Groq (fast-filter) | console.groq.com → API Keys | ✅ | ✅ |
| `AIRADAR_MISTRAL_API_KEY` | LLM fallback — Mistral (deep-extract fallback) | console.mistral.ai → API Keys | ✅ | ✅ |
| `AIRADAR_CEREBRAS_API_KEY` | LLM fallback — Cerebras (fast-filter fallback) | cloud.cerebras.ai → API Keys | ✅ | ✅ |
| `AIRADAR_OPENROUTER_API_KEY` | LLM fallback — OpenRouter (универсальный, `:free` модели) | openrouter.ai → Keys | ✅ | ✅ |
| `AIRADAR_TG_BOT_TOKEN` | Telegram-нотификации о новых high-score офферах (`notify`) | @BotFather | ✅ | ✅ (как `TG_BOT_TOKEN`) |
| `AIRADAR_TG_CHAT_ID` | чат/канал для нотификаций | @BotFather / get chat id | ✅ | ✅ (как `TG_CHAT_ID`) |
| `AIRADAR_TG_GROUP_CHAT_ID` | форум-супергруппа для постинга по темам (если задана — `notify` шлёт в темы вместо одиночного чата) | id супергруппы вида `-100…` | ✅ | ✅ (как `TG_GROUP_CHAT_ID`) |
| `AIRADAR_CF_*` / `CLOUDFLARE_API_TOKEN` | wrangler-доступ к D1 (export/execute), деплой воркера | dash.cloudflare.com → My Profile → API Tokens (D1 Edit) | ❌ | ✅ (как `CF_API_TOKEN`) |
| `CLOUDFLARE_ACCOUNT_ID` | account scope для wrangler | dash.cloudflare.com → Workers Overview | ❌ | ✅ (как `CF_ACCOUNT_ID`) |

> **Telegram-нотификации vs Telegram-ingest** — это разные вещи: `AIRADAR_TG_BOT_TOKEN`/
> `_CHAT_ID` (бот шлёт уведомления) нужны и на VDS, и в Actions; а `AIRADAR_TG_API_ID`/
> `_HASH` (Telethon читает каналы) — это stream-коллектор и имеет смысл только на VDS
> (см. §14).

> **Постинг в форум-группу по темам.** Если задан `AIRADAR_TG_GROUP_CHAT_ID`,
> `notify` шлёт офферы в **темы** супергруппы (вместо одиночного `TG_CHAT_ID`):
> 🤖 ИИ-сервисы и агенты · 🎁 Халява и акции · 📡 Из других каналов. Требуется:
> бот добавлен в группу **админом** с правом «Manage topics», у группы включён
> режим «Темы». Темы создаются автоматически: CI-шаг `tg-setup` создаёт их и
> сохраняет `message_thread_id` в таблицу `sources` (до дампа в D1, чтобы id
> переживали запуски и темы не плодились). Можно задать id вручную через
> `AIRADAR_TG_TOPIC_AI_SERVICES` / `_FREEBIES` / `_FORWARDED` (тогда авто-создание
> для них пропускается). Фильтр включения: `(score ≥ AIRADAR_NOTIFY_MIN_SCORE и
> confidence ≥ AIRADAR_NOTIFY_MIN_CONFIDENCE)` ИЛИ оффер пришёл из чужого
> TG-канала.

> **Не путать с дефолтными настройками.** Переменные `AIRADAR_LLM_BASE_URL`,
> `AIRADAR_LLM_MODEL`, `AIRADAR_LLM_PROVIDER_ORDER`, все `AIRADAR_*_MODEL`,
> `AIRADAR_RSSHUB_URL`, `AIRADAR_TG_SESSION` — это **не секреты**, у них есть
> осмысленные дефолты в коде. Их НЕ нужно прокидывать из пустых GitHub-секретов
> (пустое значение затёрло бы рабочий дефолт). Меняйте их только осознанно через
> `.env` на VDS.

---

## 14. Два рантайма: VDS vs Cloudflare — что где исполняется

Проект поддерживает **два независимых рабочих сценария**. Один и тот же Python-код,
но разный способ запуска, хранения и отдачи данных.

### 1. VDS / Docker (long-lived процесс)
- `AIRADAR_PLATFORM=local`, БД — **SQLite-файл** (`AIRADAR_DB_URL=sqlite:////data/aiapiradar.db`).
- `runner=process` — APScheduler крутит **постоянный цикл** (`sched/apscheduler_impl.py`):
  коллекторы по интервалам, discovery-job, watchdog, notifier.
- Это **единственное место, где работают stream-коллекторы** — `certstream`
  (слушает CT-логи) и `telegram` (Telethon-сессия). Им нужен живой процесс, в
  батч/CI они не запускаются.
- API отдаёт **FastAPI `web.py`** (дашборд + JSON), `docker compose` поднимает
  сервисы `scheduler` и `web`, делящие один том `radar_data`.
- **Cron** = интервалы APScheduler внутри процесса.
- **Секреты** живут в `.env` (Docker-compose монтирует его через `env_file`).

### 2. Cloudflare (текущий продакшн)
- Python-пайплайн выполняется **в GitHub Actions** по часовому cron'у
  (`.github/workflows/collectors.yml`), всё так же с `AIRADAR_PLATFORM=local` +
  SQLite внутри раннера.
- На границах раннер синхронит данные с D1 через wrangler: `wrangler d1 export`
  (D1 → локальный SQLite в начале) и `wrangler d1 execute --file=seed.sql`
  (локальный SQLite → D1 в конце).
- Данные из **Cloudflare D1** читает **TypeScript Worker** (`worker/src/index.ts`)
  и отдаёт API; фронтенд — Next.js на Cloudflare Pages.
- Здесь **stream-коллекторы НЕ работают** — CI-проход батчевый и эфемерный,
  поэтому гоняются только poll-коллекторы. `certstream`/`telegram` остаются за VDS.
- **Cron** = расписание GitHub Actions (`schedule: cron`).
- **Секреты** живут в **GitHub repo secrets** (см. §13).

### ⚠️ Про `AIRADAR_PLATFORM=cloudflare`
В конфиге существует режим `AIRADAR_PLATFORM=cloudflare` (Python обращается к D1
напрямую через REST — `db/d1.py` + batch-runner). Это
**альтернативный/экспериментальный путь** и **НЕ является текущим продакшном.**
Прод сегодня — это связка из сценария 2 выше: **GitHub Actions + SQLite +
wrangler-синхронизация**, а Worker лишь читает D1. Не путать.

### Карта «что где»

| Аспект | VDS / Docker | Cloudflare (продакшн) |
|--------|--------------|------------------------|
| База данных | SQLite-файл (`/data/aiapiradar.db`) | Cloudflare D1 (через wrangler-sync) |
| Планировщик | APScheduler (интервалы в процессе) | GitHub Actions cron (часовой) |
| Stream-коллекторы (`certstream`, `telegram`) | ✅ да | ❌ нет (только poll-коллекторы) |
| API-сервер | FastAPI `web.py` | TS Worker (`worker/src/index.ts`) |
| Где живут секреты | `.env` (`env_file` в compose) | GitHub repo secrets |
| `AIRADAR_PLATFORM` | `local` | `local` (в раннере); `cloudflare` — отдельный экспериментальный режим |
