...
def test_pipeline_persists_and_dedups(db_env):
    ...
    with get_db() as db:
        assert db.execute("SELECT COUNT(*) AS n FROM services")[0]["n"] == 1
        assert db.execute("SELECT COUNT(*) AS n FROM offers")[0]["n"] == 1   # deduped to a single offer
        assert db.execute("SELECT COUNT(*) AS n FROM signals")[0]["n"] == 2  # two unique stored signals


def test_pipeline_over_real_export(db_env):
